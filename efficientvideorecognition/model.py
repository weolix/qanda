...
from efficientvideorecognition.vision_transformer import QuickGELU, Attention
from efficientvideorecognition.weight_loaders import weight_loader_fn_dict
from efficientvideorecognition.vision_transformer import (
...
class TemporalCrossAttention(nn.Module):
    ...
    def __init__(
        ...
        self.w2 = nn.Parameter(torch.zeros([w_size, feature_dim])) # 位置偏置量
        # idx_tensor： 位置index
        idx_tensor = torch.zeros([np.prod(spatial_size) for _ in (0, 1)], dtype=torch.long) # shape: torch.Size([196, 196])
...
class EVLTransformer(nn.Module):
    ...
    def __init__(
        ...
        backbone_path: str = '/home/user/.cache/clip/ViT-B-16.pt',
        ...
        self.proj = nn.Sequential(
            ...
            nn.Linear(backbone_feature_dim, out_dim),
        )
    ...
    def forward(self, x: torch.Tensor):
        # x : torch.Size([batchsize, frames, 3, 224, 224])
        backbone = self._get_backbone(x)
        batch_size = x.shape[0]
        x = x.reshape(-1, self.num_frames, 3, 224, 224)
        B, T, C, H, W = x.size()  
        x = x.flatten(0, 1)
        ...
        features[0]: 
        dict(
        'q':        tensor of torch.Size([4, 32, 197, 12, 64])
                    tensor of torch.Size([batchsize*video_clips, 32, 197, 12, 64])
        'v':        tensor of torch.Size([4, 32, 197, 12, 64])
        'v':        tensor of torch.Size([4, 32, 197, 12, 64])
        'attn_out': tensor of torch.Size([4, 32, 197, 768])
        'out':      tensor of torch.Size([4, 32, 197, 768])
        ([batch size, num of frames, num of patchs per frame+[CLS], patch_size)
        )
        '''
        x = self.decoder(features) # (4, 1, 768)
        x = self.proj(x[:, 0, :]).reshape(batch_size, -1, 512)
 
        return x.mean(1, keepdim=False)
    
if __name__ == '__main__':
    tca = TemporalCrossAttention()
    evldec = EVLDecoder()
    evl = EVLTransformer().cuda()
    one = torch.ones((4,3,32,224,224)).cuda()

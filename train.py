...
class NTIRE_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        video_data = dbclip.read_video(video_path, yml_path=r"/home/data1/xxr/NTIREdataset/aigcc.yml")
...
def train(model, train_set, val_set = None):
    train_loader  = DataLoader(train_set, 16, shuffle=True, num_workers=2, pin_memory=True)
    val_loader    = DataLoader(val_set, 16, num_workers=2, pin_memory=True)
    optimizer = torch.optim.AdamW(model.parameters(), lr=0.0006)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, 30, eta_min=0.00005)

    wandb.init(
    project="aicg-vqa",
    config={
        
        "architecture": "newModel with fastvqa",
        "dataset": "aicg-vqa",
        "epochs": 30,
    }
    )
    ...
    for epoch in range(30):
        ...
        if val_set is None:
            torch.save(model.state_dict(), f'pretrain/now_{epoch}.pt')
...
def generate_result(model, test_data):
    ...
    with open(r"/home/data1/xxr/NTIREdataset\output.txt", 'w', newline='',encoding='utf-8') as file:
        writer = csv.writer(file)
        for pr, name in zip(val_prs, vid_names):
            writer.writerow([name, pr])
    
    with open(r"/home/data1/xxr/NTIREdataset\readme.txt", 'w', encoding='utf-8') as file:
...
if __name__ == '__main__':
    def main():
        ...
        # max_files = r'/home/data1/xxr/NTIREdataset\maxvqa'
        ...
        aigc_trainset = NTIRE_Dataset(r'/home/data1/xxr/NTIREdataset')
        aigc_valset = NTIRE_Dataset(r'/home/data1/xxr/NTIREdataset', mode='val')
        ...
        aigc_trainset, aigc_valset = random_split(aigc_trainset, (0.9, 0.1), generator42)
